...
const TranslatePage: FC = () => {
  ...
  const depRerenderPendingRef = useRef(false)
  ...
  const runMarkdownRender = useCallback(async function runMarkdownRender() {
    ...
    if (!isMountedRef.current || !enableMarkdownRef.current) return
    // A dep-only rerender (theme switch) skipped mid-flight must not be lost.
    if (depRerenderPendingRef.current) {
      depRerenderPendingRef.current = false
  ...
  useEffect(() => {
    ...
    if (delay === 0) {
      ...
      if (renderInFlightRef.current && !contentChanged) {
        depRerenderPendingRef.current = true
      } else {
        void runMarkdownRender()
      }
